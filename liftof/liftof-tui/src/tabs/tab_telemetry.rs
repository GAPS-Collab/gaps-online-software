...
pub struct TelemetryTab<'a> {
  ...
  pub merged_queue  : VecDeque<TelemetryEvent>,
  pub header_queue  : VecDeque<TelemetryPacketHeader>,
...
impl TelemetryTab<'_> {
  pub fn new(tp_sender   : Option<Sender<TofPacket>>,
    ...
    Self {
      ...
      tele_recv    : tele_recv,
      queue_size   : 20000,
      merged_queue : VecDeque::<TelemetryEvent>::new(),
      header_queue : VecDeque::<TelemetryPacketHeader>::new(),
      tp_sender    : tp_sender,
  ...
  pub fn receive_packet(&mut self) -> Result<(), SerializationError> {  
    match self.tele_recv.try_recv() {
      ...
      Ok(packet) => {
        ...
        let telly_ptype = TelemetryPacketType::from(packet.header.packet_type);
        ...
        if packet.header.packet_type == TelemetryPacketType::BoringEvent 
          ...
          match TelemetryEvent::from_bytestream(&packet.payload, &mut 0) {
            Err(err) => {
              error!("Unable to decode MergedEvent Telemetry packet of type {}! The expected size is {}, but the payload len is {}! {err}", packet.header.packet_type, expected_size - TelemetryPacketHeader::SIZE, packet.payload.len());
            }
            Ok(me) => {
              if let Some(sender) = &self.tp_sender {
