        ...
        match TofPacket::from_bytestream(&buffer, &mut 4) { 
          ...
          Ok(tp) => {
            ...
            match tp.packet_type {
              TofPacketType::RBEvent | TofPacketType::RBEventMemoryView => {
                //let mut event = RBEvent::from(&tp);
                let mut event : RBEvent;
              ...
              TofPacketType::RBCalibration => {
                let mut flight_v = RBCalibrationFlightV::new();
                let mut flight_t = RBCalibrationFlightT::new(); 
