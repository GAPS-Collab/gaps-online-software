...
-> Result<(), CalibrationError> {
  ...
  //}
  let calib_pack = calibration.pack();
...
//pub fn send_preamp_bias_set_all(bias_voltage: u16) -> Result<(), RunError> {
...
//pub fn send_preamp_bias_set(preamp_id: u8, bias_voltage: u16) -> Result<(), RunError> {
...
pub fn send_ltb_all_thresholds_set() -> Result<(), RunError> {
  match ltb_threshold::set_default_threshold() {
    Ok(_) => return Ok(()),
    Err(_) => {
      error!("Unable to set preamp bias! Error LTBThresholdError!");
      return Err(RunError::CanNotConnectToMyOwnZMQSocket)
    }
  };
}


pub fn send_ltb_all_thresholds_reset() -> Result<(), RunError> {
...
pub fn send_ltb_threshold_set(_ltb_id: u8, threshold: LTBThreshold, threshold_level: u16) -> Result<(), RunError> {
  // TODO add check for LTB of interest
  match ltb_threshold::set_threshold(threshold as u8, threshold_level as f32) {
    Ok(_)  => {
      info!("LTB threshold set!");
    },
    Err(_) => {
      error!("Unable to set preamp bias! Error LTBThresholdError!");
    }
