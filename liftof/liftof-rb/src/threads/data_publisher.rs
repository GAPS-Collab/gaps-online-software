//! Make sure everything gets sent out over zmq to the TOF CPU (or 
...
pub fn data_publisher(data           : &Receiver<TofPacket>,
  ...
  loop {
    ...
    match data.recv() {
      ...
      Ok(packet)    => {
        if matches!(packet.packet_type, TofPacketType::RBEvent) {
        ...
        if packet.packet_type == TofPacketType::RBCalibration {
