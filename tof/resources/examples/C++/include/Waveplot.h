...
public:
  ...
  Waveplot (double *data, double *time, int chnl, int flag = 0);
  ...
  int     Add (Waveplot * rhs);
  void    FindAverageWaveform(void);
  int     RestoreOriginalTrace(int lo, int size=9999);
  void    SetThreshold(float PmtThreshold);
  int     GetWaveSize(void){return wf_size;}
  void    PlotWaveform(int npoints, float x_lo=-9999, float x_hi=-9999,
		       float y_lo=-9999, float y_hi=-9999);
  void    PlotFit(void);
  void    SetPeakPlot(int flag) {peak_plot = flag;}
  ...
  int     FindNumPeaks(const char *method, int start_val=9999, int size=192);
  ...
  void    MeasurePeaksRms(int start=0, int size=9999);
  void    MeasurePeaksThresh(float start=0, float size=9999, 
			     int th_type=CONSTANT);
...
private:
  ...
  int     added_waveforms;           // Number of Waveforms added to this one
  ...
  int     wf_modified;               // Have we altered the digi trace
  double  *WaveDataOrig;             // Original waveform trace
  ...
  int     *sat_pk;         // Was this peak saturated?
  ...
  // Stuff related to the peaks
  void    AllocatePeaks();
  double  CalculateWidth(int pk, float level);
