  ...
  'main: loop {
    ...
    if timer.elapsed().as_secs() >= cmd_interval {
      ...
      match incoming.try_recv() { // we want to be non-blocking here
        Err(err) => {
          trace!("did not receive a new command over cb channel! {err}");
          debug!("Listening for flight CPU comms");
          match cmd_socket.recv_bytes(zmq::DONTWAIT) {
            Err(err) => {
              trace!("No new packet, err {err}");
              sleep(sleep_time);
              continue;
      ...
      PacketType::TofCommand => {
        ...
        match TofCommand::from_bytestream(&pack.payload, &mut 0) {
          ...
          Ok(cmd)  => {
            ...
            let mut resp_socket : Option<zmq::Socket> = None;
            match resp_socket {
              None => (),
              Some(ref mut _resp_socket) => {
                info!("Will set up 0MQ PUB socket to reply to flight cpu commands at address {flight_address_pub}");
                _resp_socket.connect(flight_address_pub).expect("Unable to bind to data (PUB) socket {data_adress}");
