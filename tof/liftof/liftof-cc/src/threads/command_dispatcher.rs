...
pub fn command_dispatcher(settings        : CommandDispatcherSettings,
  ...
  // FIXME - do we want a topic here?
  cmd_receiver.set_subscribe(b"").expect("Unable to subscribe to empty topic!");
  cmd_receiver.connect(&cc_pub_addr).expect("Unable to subscribe to flight computer PUB");
  //info!("ZMQ SUB Socket for flight cpu listener bound to {fc_sub_addr}");
  info!("Listening on {cc_pub_addr}!");
  ...
  loop {
    ...
    match cmd_receiver.connect(&cc_pub_addr) {
      Ok(_)    => (),
      Err(err) => {
          ...
          Ok(packet) => {
            ...
            match packet.packet_type {
              PacketType::BfswAckPacket => {
                match tof_ack_sender.send(packet) {
                  Err(err) => {
                    error!("Unable to send ACK packet! {err}");
                  }
                  Ok(_)    => ()
