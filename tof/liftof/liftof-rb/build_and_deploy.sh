...
compile_and_deploy_target() {
  CARGO_TARGET_ARMV7_UNKNOWN_LINUX_GNUEABI_RUSTFLAGS="-C relocation-model=dynamic-no-pic -C target-feature=+crt-static" cross build --bin $1 --target=armv7-unknown-linux-musleabi --release && scp ../target/armv7-unknown-linux-musleabi/release/$1 $2: 
}

# first delete everything, since there might be remains of a previously issued cargo check
rm -rf ../target/armv7-unknown*

#compile_and_deploy_target liftof-rb ucla-rb101
#compile_and_deploy_target liftof-rb ucla-rb102
compile_and_deploy_target liftof-rb ucla-rb103
compile_and_deploy_target liftof-rb ucla-rb104
