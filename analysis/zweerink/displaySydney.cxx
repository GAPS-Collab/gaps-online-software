...
int main(int argc, char *argv[]){
  ...
  for (auto const &p : packets) {
    ...
    if (!ev_res.is_ok()) {
      spdlog::error("Unable to unpack TofEvent. Deserialization error!");
      continue;
    }
...
void plotall(int n_ch, int nrbs) {
  ...
  if (plot_flag == ALLCH) {
    ...
    //cm->Divide(x, y, 1.0e-5, 1.0e-5);
    //cm->Divide(3, 3, 1.0e-5, 1.0e-5);
    //cm->Divide(, nrbs, 1.0e-5, 1.0e-5);
    // This number determines how many RB channels to plot 
    int rb_ch_hi = 3;
    cm->Divide(rb_ch_hi, nrbs, 1.0e-5, 1.0e-5);
    int ctr = 0, pl_num, ch_num=0;
    int rbid;
    for (int i = 0; i < n_ch; i++) {
      //if (wave[i] != NULL) {
      if (wave[i] != NULL && i%8 <rb_ch_hi) {
