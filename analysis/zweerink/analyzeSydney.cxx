...
int main(int argc, char *argv[]){
  ...
  if (calname != "") {
    ...
    snprintf(pname,450, "ls %s/rb_*.cali.tof.gaps", calname.c_str());
    FILE *fp = popen(pname, "r");
    while (fscanf(fp,"%s", line) != EOF) {
      std::string c_name(line);          // Calib file found
      int position = c_name.find("rb_");  // Find "RB" in the name
      std::string rbstr = c_name.substr(position+3, 2); // Extract RB num
  ...
  for (auto const &p : packets) {
	...
	if (!ev_res.is_ok()) {
      spdlog::error("Unpacking of TofEvent failed!");
      continue;
    }
	...
	for (auto const &rbid : ev.get_rbids()) {
	  ...
	  if (calname != "") { // For combined data all boards calibrated
	    ...
	    volts = cali[rbid].voltages(rb_event, true);
		...
		std::ofstream myfile1;
        	myfile1.open ("Peds.csv", std::ios::app);
        	myfile1 << evt_ctr << "," << cw << "," << Ped[cw] << "," << PedRMS[cw] << std::endl;
        	myfile1.close();
		...
		std::ofstream myfile;
        	...
        	myfile << evt_ctr << "," << cw << "," << TDC[cw] << "," << Qint[cw] << std::endl;
        	myfile.close();	
