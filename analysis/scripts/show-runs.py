...
outfile = open('run-table.md', 'w')
outfile.write('Run id\t trigger \t prescale \t trace suppressed \t size \t begin time \t runtime\n')
outfile.write('--------------------------------------------------------------------------------=\n')
for run in sorted(runinfo.keys()):
    ...
    outfile.write(f'{str(run)}\t {runinfo[run]["trigger_type"]}\t {runinfo[run]["prescale"]:.4f} \t {str(runinfo[run]["rate"])}\t {str(runinfo[run]["trace-suppressed"])} \t {str(runinfo[run]["size"])} \t {str(runinfo[run]["begin-time"])}\t {str(runinfo[run]["runtime"])}\n')
