...
mod tracker_mask;
pub use tracker_mask::{
  TrackerStripMask,
  create_trk_mask_table
};

mod tracker_pedestal;
pub use tracker_pedestal::create_trk_pedestal_table;

mod tracker_transfer_fn;
pub use tracker_transfer_fn::{
...
impl TrackerStrip {
  ...
  /// Factory method for the strip id following tracker convention
  pub fn create_stripid(layer : u8, row :u8, module : u8, channel : u8) -> u32 {
...
impl TrackerStripPedestal {
  ...
  pub fn parse_from_file<P: AsRef<Path>>(path: P) -> io::Result<Vec<Self>> {
    ...
    let mut pedestals = Vec::<Self>::new();
    ...
    for line in reader.lines() {
      ...
      let mut ped = TrackerStripPedestal::new();
      ...
      if parts.len() == 6 {
        ...
        let channel  = parts[3].parse::<u8>()
          .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;
        ped.strip_id  = TrackerStrip::create_stripid(layer, row, module, channel) as i32; 
        ped.volume_id = *hid_vid_map.get(&(ped.strip_id as u32)).unwrap() as i64; // critical error is good here,
                                                               // should alert the user
        ped.pedestal_mean  = parts[4].parse::<f32>()
          .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;
        ped.pedestal_sigma = parts[5].parse::<f32>()
          .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;
        pedestals.push(ped); 
      }
    }
...
impl TrackerStripPedestal {
  ...
  #[getter]
  #[pyo3(name="name")]
  fn get_name_py (&self) -> Option<String> {
    self.name.clone()
  }
