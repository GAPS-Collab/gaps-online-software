...
pub static GENERIC_ONLINE_FILE_PATTERN : &str = r"Run\d+_\d+\.(\d{6})_(\d{6})(UTC)?(\.tofsum|\.tof)?\.gaps$";
pub static GENERIC_ONLINE_FILE_PATTERN_CAPTURE : &str = r"Run(?P<run>\d+)_(?P<subrun>\d+)\.(?P<utctime>\d{6}_\d{6})(UTC)?(\.tofsum|\.tof)?\.gaps$";
pub static GENERIC_TELEMETRY_FILE_PATTERN_CAPTURE : &str = r"RAW(?P<utctime>\d{6}_\d{6})(UTC)?.bin$";
